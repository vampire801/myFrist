...
using MahjongLobby_SX_Changzhi.Data;
using System.Text;
using System.Collections.Generic;
using MahjongLobby_SX_Changzhi.Network.Message;
using MahjongLobby_SX_Changzhi.Network;
using System.Net;
...
namespace MahjongLobby_SX_Changzhi
{
    ...
    public class SDKManager : MonoBehaviour
    {
        ...
        private void InItAll()
        {
            if (Application.platform == RuntimePlatform.Android)
            {
                sxJc = new AndroidJavaClass("com.ibluejoy.shuangxi.wxapi.WXEntryActivity");
            }
        ...
        {
            WXLoginPanelData ld = MahjongLobby_SX_Changzhi.GameData.Instance.WXLoginPanelData;
            ld.AuthonState = 0;
        ...
        {
            MahjongGame_SX_Changzhi.UIMainView.Instance.PlayerPlayingPanel._imageBattery.fillAmount = int.Parse(num) * 0.01f;
        }
        ...
        public void SharedSuccess(string str)
        {
            ...
            int m_ifrom = 0;
            if (PlayerPrefs.HasKey("GameActivie"))
            {
                m_ifrom = PlayerPrefs.GetInt("GameActivie");
            }
            ...
            else
            {
                ...
                {
                    MahjongGame_SX_Changzhi.UIMainView.Instance.GameResultPanel.RPButton.SetActive(false);
                    Debug.LogWarning("分享完成后关闭大赢家和炮王红包按钮" + m_ifrom);
                }
                Debug.LogWarning("游戏分享");
                MahjongGame_SX_Changzhi.Network.Message.NetMsg.ClientShareSuccessReq msg = new MahjongGame_SX_Changzhi.Network.Message.NetMsg.ClientShareSuccessReq();
                msg.iUserID = MahjongCommonMethod.Instance.iUserid;
                msg.byShareType = (byte)m_ifrom;
                Debug.LogWarning("byShareType:" + msg.byShareType);
                MahjongGame_SX_Changzhi.Network.NetworkMgr.Instance.GameServer.SendShareSuccessReq(msg);
            }
            //隐藏分享按钮的引导
        ...
        IEnumerator GetReTokenReq(string REFRESH_TOKEN)
        {
            ...
            //保存accessToken
            PlayerPrefs.SetString(szrefresh_token, elac.refresh_token);
        ...
        public void WXOpenAuthReq()
        {
            ...
            ld.AuthonState = 1;
            if (PlayerPrefs.HasKey(instance.szLastday + MahjongCommonMethod.EVERYDAY.HEADIMG_UPDATE))
            {
        ...
        public void SendAuthReqTyp2()
        {
            ...
            msg.szToken = PlayerPrefs.GetString(Instance.szrefresh_token);   //
            ...
            {
                string[] strs = PlayerPrefs.GetString(Instance.iUserId_iAuthType_ServerType).Split('_');
        ...
        public IEnumerator LoadQRcodeAndDisplay(string url, int type, int mode, GameObject[] obj)
        {
            ...
            while (xx++ < 4)
            {
                using (UnityWebRequest www = UnityWebRequest.GetTexture(url))
                {
                    ...
                    // int width = (int)MahjongGame_SX_Changzhi.UIMainView.Instance.GameResultPanel._rawImage.uvRect.width;
                    //  int height = (int)MahjongGame_SX_Changzhi.UIMainView.Instance.GameResultPanel._rawImage.uvRect.height;
                    // texture2D = ScaleTexture(DownloadHandlerTexture.GetContent(www), width, height);
                    texture2D = DownloadHandlerTexture.GetContent(www);

                    Debug.Log("复制图片");
                    //  MahjongGame_SX_Changzhi.UIMainView.Instance.GameResultPanel._rawImage.material.mainTexture = texture2D;
                    //  MahjongGame_SX_Changzhi.UIMainView.Instance.GameResultPanel._rawImage.enabled = true;
                    if (texture2D != null)
                    {
        ...
        IEnumerator GenerateShareImage(Texture2D _qRcode, int type, int mode, GameObject[] obj)
        {
            ...
            //取消显示二维码
            //二维码重设长宽
            int code_width = (int)MahjongGame_SX_Changzhi.UIMainView.Instance.GameResultPanel._rawImage.rectTransform.sizeDelta.x;
            int code_height = (int)MahjongGame_SX_Changzhi.UIMainView.Instance.GameResultPanel._rawImage.rectTransform.sizeDelta.y;
            ...
            MahjongGame_SX_Changzhi.UIMainView.Instance.GameResultPanel.ShowContent[2].gameObject.SetActive(false);
            for (int i = 0; i < obj.Length; i++)
            {
        ...
        byte[] CaptureCamera(RawImage _qRcode, Rect rect, Camera MainCamera, Camera camera2)
        {
            ...
            Application.CaptureScreenshot("NewScreenshot.jpg");
